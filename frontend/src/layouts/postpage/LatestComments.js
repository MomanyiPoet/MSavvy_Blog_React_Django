        ...
        fetch('http://localhost:8000/api/comment/')
            ...
            .then(data => {
                ...
                const promises = data.map(comment => {
                    return fetch(`http://localhost:8000/api/article/${comment.article}/`)
                        .then(response => response.json())
                        .then(articleData => {
            ...
            <div className="row py-2" key={index}>
                ...
                <div className="row">
                    ...
                    <h6 className="px-xl-2">{comment.fullname} on article <span className='font-italic text-md text-secondary'>"{comment.article ? comment.article : 'Unknown'}"</span></h6>
                        <h6 className="px-xl-2 text-xs opacity-6 text-end">{new Date(comment.createdat).toLocaleString('en-US', { year: 'numeric', month: '2-digit', day: '2-digit'})}</h6>
                    </div>
                </div>
                <div>
